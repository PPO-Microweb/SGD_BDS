...
var PPOFixed = {
    ...
    columns: function (col) {
        ...
        summaries.each(function (i) {
            ...
            if(is_fixed_menu){
                margin_top += jQuery(".ppo_menu").outerHeight(true);
            }
...
//    fakewaffle.responsiveTabs(['xs', 'sm']);
    ...
    jQuery('#boxTab a, #myTab a').click(function (e) {
        e.preventDefault();
        jQuery('input[name="trantype"]').val(jQuery(this).data('id'));
    ...
    // User Levels Carousel
    if(jQuery(".user-levels .owl-carousel").length > 0){
        jQuery(".user-levels .owl-carousel").owlCarousel({
            autoplay: false,
            autoplayHoverPause: true,
            loop: false,
            margin: 30,
    ...
    jQuery(".list_product .item_product .row-pro .manage-tools .icon-toggle").click(function(){
    ...
    jQuery(document).mouseup(function (e) {
        var manage_tools = jQuery(".list_product .item_product .row-pro .manage-tools .tools-wrap");
        var manage_tools_toggle = jQuery(".list_product .item_product .row-pro .manage-tools .icon-toggle");
        if (!manage_tools.is(e.target) && !manage_tools_toggle.is(e.target) && manage_tools.has(e.target).length === 0) {
            manage_tools.hide();
        }
    });
    
    if(viewport_width < 992){
        jQuery(".project-filter form").removeClass('open').addClass('close');
    }
    jQuery(".project-filter .label-toggle").click(function(){
        if(jQuery(".project-filter form").hasClass('open')){
            jQuery(".project-filter form").removeClass('open').addClass('close');
        } else {
            jQuery(".project-filter form").removeClass('close').addClass('open');
        }
    });
    
    if(jQuery(".main_menu>li.trantype-sell, .menu-mobile li.trantype-sell").length > 0){
        jQuery(".main_menu>li.trantype-sell, .menu-mobile li.trantype-sell").find('a').each(function(){
            if(jQuery(this).attr('href').trim().length > 0){
                jQuery(this).attr('href', jQuery(this).attr('href') + "?trantype=sell");
            }
        });
    }
    if(jQuery(".main_menu>li.trantype-rent, .menu-mobile li.trantype-rent").length > 0){
        jQuery(".main_menu>li.trantype-rent, .menu-mobile li.trantype-rent").find('a').each(function(){
            if(jQuery(this).attr('href').trim().length > 0){
                jQuery(this).attr('href', jQuery(this).attr('href') + "?trantype=rent");
            }
        });
    }
    if(jQuery(".main_menu>li.trantype-invest, .menu-mobile li.trantype-invest").length > 0){
        jQuery(".main_menu>li.trantype-invest, .menu-mobile li.trantype-invest").find('a').each(function(){
            if(jQuery(this).attr('href').trim().length > 0){
                jQuery(this).attr('href', jQuery(this).attr('href') + "?trantype=invest");
            }
        });
    }
