...
jQuery(document).ready(function ($) {
    ...
    jQuery(".list_product .item_product .share-pro .save-post, .carousel-products-widget .item .tools .save-post, .list_product .item_product .tools .save-post, .single_product .short_info .save-post").click(function(){
        displayAjaxLoading(true);
    ...
    // Add post to Compare
    jQuery(".list_product .item_product .share-pro .compare-post, .carousel-products-widget .item .tools .compare-post, .list_product .item_product .tools .compare-post, .single_product .short_info .compare-post").click(function(){
        displayAjaxLoading(true);
        var post_id = jQuery(this).data('id');
        jQuery.ajax({
            url: ajaxurl, type: "POST", dataType: "json", cache: false,
    ...
    // Lien he nguoi dang tin bds
    jQuery(".widget-contact-publisher form").submit(function(){
        var frmContactPublisher = this;
        jQuery(frmContactPublisher).find('input[type="submit"]').val('Đang gửi...').attr('disabled', 'disabled');
        jQuery.ajax({
            url: ajaxurl, type: "POST", dataType: "json", cache: false,
            data: jQuery(frmContactPublisher).serialize(),
            success: function(response, textStatus, XMLHttpRequest){
                if(response && response.status === 'success'){
                    displayBarNotification(true, 'nSuccess', response.message);
                    frmContactPublisher.reset();
                } else if(response.status === 'error'){
                    displayBarNotification(true, 'nWarning', response.message);
                }
            },
            ...
            complete:function(){
                jQuery('.widget-contact-publisher form input[type="submit"]').val('Gửi liên hệ').removeAttr('disabled');
            }
        });
        return false;
    });
    ...
    // Upgrade account
    jQuery("form[id^='frmUserLevel_']").submit(function(){
        displayAjaxLoading(true);
        var frmUserLevel = this;
        jQuery.ajax({
            url: ajaxurl, type: "POST", dataType: "json", cache: false,
            data: jQuery(frmUserLevel).serialize(),
            success: function(response, textStatus, XMLHttpRequest){
                if(response){
    ...
    // Delete Post
    jQuery(".list_product .item_product .row-pro .manage-tools .tools-wrap .act-del").click(function(){
        if(confirm('Bạn đã chắc chắn chưa?')){
            displayAjaxLoading(true);
            jQuery.ajax({
                url: ajaxurl, type: "POST", dataType: "json", cache: false,
                data: {
                    action: 'ppo_delete_post',
    ...
    // Up VIP Post
    jQuery(".list_product .item_product .row-pro .manage-tools .tools-wrap .act-upvip").click(function(){
        if(confirm('Bạn đã chắc chắn chưa?')){
            displayAjaxLoading(true);
            jQuery.ajax({
                url: ajaxurl, type: "POST", dataType: "json", cache: false,
                data: {
                    action: 'ppo_upvip_post',
                    post_id: jQuery(this).data('id')
                },
                success: function(response, textStatus, XMLHttpRequest){
                    if(response){
    ...
    // Down VIP Post
    jQuery(".list_product .item_product .row-pro .manage-tools .tools-wrap .act-downvip").click(function(){
        if(confirm('Bạn đã chắc chắn chưa?')){
            displayAjaxLoading(true);
            jQuery.ajax({
                url: ajaxurl, type: "POST", dataType: "json", cache: false,
                data: {
                    action: 'ppo_downvip_post',
                    post_id: jQuery(this).data('id')
                },
                success: function(response, textStatus, XMLHttpRequest){
                    if(response){
    ...
    jQuery('.starrr').on('starrr:change', function (e, value) {
        if(value){
            jQuery(".user-rating .display-rating").addClass('hide');
            jQuery(".user-rating .set-rating").removeClass('hide');
            jQuery.ajax({
                url: ajaxurl, type: "POST", dataType: "json", cache: false,
                data: {
                    action: 'add_user_rating',
                    user_id: jQuery(".starrr").data('id'),
                    value: value
                },
                success: function(response, textStatus, XMLHttpRequest){
                    if(response){
                        if(response.rating && response.rating.length > 0){
                            jQuery(".user-rating .display-rating").html(response.rating);
                        }
                        jQuery(".user-rating .display-rating").removeClass('hide');
                        jQuery(".user-rating .set-rating").addClass('hide');
                        jQuery('.your-choice-was').html(response.message).show();
                        setTimeout(function(){
                            jQuery('.your-choice-was').addClass('hide');
                        }, 2000);
                    }
                },
                error: function(MLHttpRequest, textStatus, errorThrown){
